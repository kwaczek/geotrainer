...
// API Pole interface as returned from the database
interface ApiPole {
  _id: string;
  imageUrl: string;
  description: string;
...
// Display Pole interface used in the UI
interface Pole {
  id: string;
  name: string;
  country: string;
...
// Transform API pole to display pole
const transformPole = (pole: ApiPole): Pole => {
  const country = pole.countries && pole.countries.length > 0 ? pole.countries[0] : null;
  ...
  return {
    id: pole._id,
    name: pole.description.split('.')[0].trim(), // Use first sentence as name
...
const fallbackPoles: Pole[] = [
  {
    id: 'fr-wood',
    name: 'French Wooden Pole',
    country: 'France',
    countryCode: 'fr',
    continent: 'Europe',
    imageUrl: 'https://placehold.co/300x400/e2e8f0/1e40af?text=French+Pole',
    description: 'Wooden pole with a distinctive top, common in France.',
    googleMapsUrl: 'https://www.google.com/maps/@48.8566969,2.3514616,3a'
  },
  {
    id: 'us-wood-double',
    name: 'US Double Wooden Pole',
    country: 'United States',
    countryCode: 'us',
    continent: 'North America',
    imageUrl: 'https://placehold.co/300x400/e2e8f0/1e40af?text=US+Pole',
    description: 'Typical double wooden utility pole found in the US.',
    googleMapsUrl: 'https://www.google.com/maps/@40.7127281,-74.0060152,3a'
  },
  {
    id: 'jp-concrete',
    name: 'Japanese Concrete Pole',
    country: 'Japan',
    countryCode: 'jp',
    continent: 'Asia',
    imageUrl: 'https://placehold.co/300x400/e2e8f0/1e40af?text=Japan+Pole',
    description: 'Common concrete utility pole seen across Japan.',
...
const STORAGE_KEY = 'geotrainer_poles_filter';
...
const PolesPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log('Poles count:', poles.length);
    console.log('First pole:', poles.length > 0 ? poles[0] : 'none');
    console.log('Filtered poles count:', filteredPoles.length);
  ...
  }, [poles, filteredPoles, selectedCountry, selectedContinent, availableContinents, availableCountries]);
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          // Will fallback to extracting from poles
        }
        ...
        try {
          ...
          // Will fallback to extracting from poles or countries
        }
        
        // Now fetch poles
        const poleResponse = await axios.get('/api/poles');
        console.log('API poles response received:', poleResponse.data);
        
        // If we got a response and it's an array, process it
        let transformedPoles: Pole[] = [];
        if (poleResponse.data && Array.isArray(poleResponse.data)) {
          console.log('Response is a direct array of poles with length:', poleResponse.data.length);
          transformedPoles = poleResponse.data.map(transformPole);
          console.log('Transformed poles:', transformedPoles.length);
          console.log('First transformed pole:', transformedPoles.length > 0 ? transformedPoles[0] : 'none');
          setPoles(transformedPoles);
          setUsingFallbackData(false);
        }
        // If the response has a poles property that's an array
        else if (poleResponse.data && poleResponse.data.poles && Array.isArray(poleResponse.data.poles)) {
          console.log('Response has poles array in data property');
          transformedPoles = poleResponse.data.poles.map(transformPole);
          console.log('Transformed poles:', transformedPoles.length);
          setPoles(transformedPoles);
          setUsingFallbackData(false);
        }
        // If none of the above formats match, use fallback
          ...
          console.warn('Unexpected API response format, using fallback data:', poleResponse.data);
          setPoles(fallbackPoles);
          setUsingFallbackData(true);
          transformedPoles = fallbackPoles;
        }
        
        // Log the result of state update
        console.log('Total poles loaded:', transformedPoles.length);

        // If we couldn't get countries from API, extract them from poles as fallback
        if (availableCountries.length === 0) {
          console.log('Extracting countries from poles as fallback...');
          const uniqueCountries = Array.from(
            new Set(transformedPoles.map(pole => pole.country))
          ).map(countryName => {
            const pole = transformedPoles.find(p => p.country === countryName);
            return {
              name: countryName,
              continent: pole?.continent || 'Unknown'
            };
          }).sort((a, b) => a.name.localeCompare(b.name));
          
          console.log('Extracted countries from poles:', uniqueCountries.length);
          setAvailableCountries(uniqueCountries);
        }
        
        // If we couldn't get continents from API, extract them from poles as fallback
        if (availableContinents.length === 0) {
          console.log('Extracting continents from poles as fallback...');
          const uniqueContinents = Array.from(
            new Set(transformedPoles.map(pole => pole.continent))
          ).filter(Boolean).sort();
          ...
          console.log('Extracted continents from poles:', uniqueContinents);
        ...
        if (availableCountries.length === 0) {
          const uniqueCountries = Array.from(
            new Set(fallbackPoles.map(pole => pole.country))
          ).map(countryName => {
            const pole = fallbackPoles.find(p => p.country === countryName);
            return {
              name: countryName,
              continent: pole?.continent || 'Unknown'
            };
        ...
        if (availableContinents.length === 0) {
          const uniqueContinents = Array.from(
            new Set(fallbackPoles.map(pole => pole.continent))
          ).filter(Boolean).sort();
  ...
  useEffect(() => {
    ...
    if (selectedContinent && selectedContinent !== 'All Continents') {
      ...
      result = result.filter(pole => {
        const match = pole.continent === selectedContinent;
        if (!match) {
          console.log('  Excluding pole:', pole.name, 'with continent:', pole.continent);
        }
        return match;
      });
    ...
    if (selectedCountry && selectedCountry !== 'All Countries') {
      ...
      result = result.filter(pole => {
        const match = pole.country === selectedCountry;
        if (!match) {
          console.log('  Excluding pole:', pole.name, 'with country:', pole.country);
        }
        return match;
      });
    ...
    if (searchTerm.trim() !== '') {
      ...
      result = result.filter(pole =>
        pole.name.toLowerCase().includes(searchLower) || 
        pole.country.toLowerCase().includes(searchLower) ||
        pole.description.toLowerCase().includes(searchLower)
      );
  ...
  }, [poles, selectedCountry, selectedContinent, searchTerm, sortBy]);
  ...
  useEffect(() => {
    const handleEscKey = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && selectedPole) {
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-20 flex flex-col items-center justify-center">
        ...
        <p className="mt-4 text-gray-600">Loading poles...</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6 text-center text-gray-800">Poles of the World</h1> 
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-yellow-700">
                Database connection unavailable. Showing sample pole data for demonstration purposes.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mb-6 text-gray-600">
        <p>Showing {filteredPoles.length} of {poles.length} poles</p>
      </div>

      {/* No results message */}
      {filteredPoles.length === 0 && ( // Changed variable name
        <div className="bg-white p-8 rounded-lg shadow-md text-center">
          <svg className="h-16 w-16 text-gray-400 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <h3 className="text-lg font-medium text-gray-900 mb-2">No poles found</h3> // Changed message
      ...
      {/* Poles Grid View */}
      {viewMode === 'grid' && filteredPoles.length > 0 && ( // Changed variable name
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredPoles.map(pole => ( // Changed variable name
            <div 
              key={pole.id}
              ...
              onClick={() => setSelectedPole(pole)} // Changed state setter
            >
              {pole.imageUrl && (
                <div className="h-48 bg-gray-100">
                  <img 
                    ...
                    onError={(e) => {
                      ...
                      target.onerror = null;
                      target.src = '/images/pole-placeholder.png'; // Changed placeholder
                    }}
                  />
                </div>
              )}
              <div className="p-4">
                <h3 className="text-lg font-semibold text-gray-800 mb-1">{pole.name}</h3>
                <p className="text-sm text-gray-600 mb-2">
                  <span className="font-medium">Country:</span> {pole.country}
                </p>
                <div className="flex flex-wrap gap-2 mt-2">
                  <ContinentBadge continent={pole.continent} />
                  {pole.countryCode && (
      ...
      {/* Poles List View */}
      {viewMode === 'list' && filteredPoles.length > 0 && ( // Changed variable name
              ...
              {filteredPoles.map((pole) => ( // Changed variable name
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-gray-900">{pole.name}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-900">
                      {pole.country}
                      {pole.countryCode && (
                        <span className="ml-1 text-xs text-gray-500">({pole.countryCode.toUpperCase()})</span>
                      )}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
      ...
      <div className="mt-12 bg-blue-50 border border-blue-200 rounded-lg p-6">
        <h2 className="text-xl font-semibold text-blue-800 mb-3">What are utility poles?</h2>
        <p className="text-blue-700 mb-4">
          Utility poles support overhead power lines, telephone wires, street lights, and other public utilities.
          Their design (material, shape, attachments) varies significantly between countries and regions, making them useful clues in location guessing games.
        </p> 
      ...
      {/* Add PoleGallery Modal */}
      {selectedPole && ( // Changed variable name
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] p-4" onClick={() => setSelectedPole(null)}> // Changed state setter
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-4">
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-xl font-semibold text-gray-800">Pole Details</h3> 
                <button 
                  className="text-gray-500 hover:text-gray-700"
                  onClick={() => setSelectedPole(null)} // Changed state setter
              ...
              <div className="mb-4">
                <img 
                  src={selectedPole.imageUrl} // Changed variable name
                  alt={`Pole in ${selectedPole.country}`} // Changed alt text
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center gap-2">
                  <span className="bg-gray-50 p-2 rounded flex items-center">
                    ...
                    <span>{selectedPole.country}</span> 
                  </span>
                  <ContinentBadge continent={selectedPole.continent} /> 
                </div>
              </div>
              
              {selectedPole.googleMapsUrl && ( // Changed variable name
                <div className="mb-4">
                  <h4 className="text-sm font-semibold text-gray-700 mb-2">Location</h4>
                  <a 
                    href={selectedPole.googleMapsUrl} // Changed variable name
                    target="_blank" 
...
export default PolesPage; // Changed component name