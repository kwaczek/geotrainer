...
async function exportBollards() {
  try {
    ...
    const outputPath = path.resolve(__dirname, 'bollards-data.ts');

    // Write to file
    fs.writeFileSync(outputPath, `export const bollards = ${JSON.stringify(formattedBollards, null, 2)};`);
    console.log(`Bollards data exported to ${outputPath}`);
