...
import { getRandomGoogleCarQuestion } from '../controllers/quiz/generators/googleCarQuestions';
import GoogleCar from '../models/GoogleCar';
import Country from '../models/Country';
import { QuizFilters } from '../controllers/quiz/types';

// Mock the GoogleCar and Country models
jest.mock('../models/GoogleCar', () => ({
  aggregate: jest.fn()
}));
...
describe('Google Car Questions Generator', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  test('should generate a valid Google Car question with no filters', async () => {
    // Mock data for a Google Car with multiple countries
    const mockGoogleCar = {
      _id: 'googlecar-1',
      imageUrl: '/uploads/google-cars/car1.jpg',
      description: 'A typical French Google Car',
    ...
    // Mock the GoogleCar.aggregate function
    (GoogleCar.aggregate as jest.Mock).mockImplementation((pipeline) => {
      // Return an object that has a lookup method
      return {
        lookup: jest.fn().mockResolvedValue([mockGoogleCar])
      };
    });
    ...
    const question = await getRandomGoogleCarQuestion();

    // Assertions
    expect(question).toBeDefined();
    expect(question.question).toBe('In which country can you find this Google Car?');
    expect(question.imageUrl).toBe('/uploads/google-cars/car1.jpg');
    ...
    // The correct option should be one of the Google Car's countries
    ...
    expect(question.metadata?.entityId).toBe('googlecar-1');
  });

  test('should apply continent filter correctly', async () => {
    // Mock data for an Asian Google Car
    const mockAsianGoogleCar = {
      _id: 'googlecar-2',
      imageUrl: '/uploads/google-cars/car2.jpg',
      description: 'A typical Japanese Google Car',
    ...
    // Mock the GoogleCar.aggregate function with continent filter
    (GoogleCar.aggregate as jest.Mock).mockImplementation((pipeline) => {
      ...
      // Return an object that has a lookup method
      return {
        lookup: jest.fn().mockResolvedValue([mockAsianGoogleCar])
      };
    });
    ...
    const question = await getRandomGoogleCarQuestion(filters);

    // Assertions
    expect(question).toBeDefined();
    expect(question.question).toBe('In which country can you find this Google Car?');
    expect(question.imageUrl).toBe('/uploads/google-cars/car2.jpg');
    expect(question.options.length).toBe(4);
    ...
    expect(question.metadata?.entityId).toBe('googlecar-2');
  });
});
