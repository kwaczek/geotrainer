...
// Sample test data for Google Cars
const testGoogleCars: Array<{
  ...
  },
  {
    ...
    description: 'A typical German Google Car',
...
beforeAll(async () => {
  ...
  // Update Google Car test data with actual country ObjectIds
...
describe('Google Car Quiz Flow', () => {
  test('Complete quiz flow: get question, answer, and verify result', async () => {
    ...
    // Check that the quiz type is cars
    expect(resultResponse.body.result).toHaveProperty('type', 'cars');
    expect(resultResponse.body.result).toHaveProperty('quizType', 'cars');
  });
});
