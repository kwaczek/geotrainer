...
describe('License Plate Quiz Flow', () => {
  test('Complete quiz flow: get question, answer, and verify result', async () => {
    ...
    // Verify question structure specific to license plates
    expect(question).toHaveProperty('imageUrl');
    expect(question.question).toBe('In which country can you find this license plate?');
    expect(question.imageUrl).toMatch(/^https:\/\/example\.com\/license-plate\d\.jpg$/);
    ...
    // Check that the quiz type is licenseplates
    expect(resultResponse.body.result).toHaveProperty('type', 'licenseplates');
    expect(resultResponse.body.result).toHaveProperty('quizType', 'licenseplates');
  });
}); 