...
describe('Road Sign Quiz Flow', () => {
  test('Complete quiz flow: get question, answer, and verify result', async () => {
    ...
    // Verify question structure specific to road signs
    ...
    // Check that the quiz type is road signs
    expect(resultResponse.body.result).toHaveProperty('type', 'roadsigns');
    expect(resultResponse.body.result).toHaveProperty('quizType', 'roadsigns');
  });
}); 