...
describe('Flag Quiz API Endpoint', () => {
  test('GET /api/quiz-questions/flags should return a valid question', async () => {
    const response = await request(app).get('/api/quiz-questions/flags');
    ...
    expect(response.body.question.question).toBe('Which country does this flag belong to?');
    
    // Verify that the image URL is formatted correctly
    expect(response.body.question.imageUrl).toMatch(/^https:\/\/flagcdn\.com\/w320\/[a-z]{2}\.png$/);
  });
  
  test('GET /api/quiz-questions/flags with continent parameter', async () => {
    ...
    const response = await request(app).get('/api/quiz-questions/flags?filters={"continent":"Europe"}');
  ...
  test('POST /api/quiz-answers/flags should process an answer correctly', async () => {
